...
class RegisterForm(FlaskForm):
    ...
    def validate_email_address(self, email_address_to_check):
        ...
        if email_address:
            raise ValidationError('E-mail address already exist, please choose a different e-mail address.')

    username=StringField(label='User Name:', validators=[Length(min=2, max=30), DataRequired()])
    email_address=StringField(label='E-mail Address:', validators=[Email(),DataRequired()])
    password1 = PasswordField(label='Password:', validators=[Length(min=6),DataRequired()])
    password2 = PasswordField(label='Re-confirm password:', validators=[EqualTo('password1'),DataRequired()])
